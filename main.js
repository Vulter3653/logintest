...
class CommentsSection extends HTMLElement {
  ...
  render() {
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .login-prompt { 
          text-align: center; padding: 30px; border: 2px dashed oklch(1 0 0 / 0.2); border-radius: 16px;
        ...
        .comment-item { 
          ...
          margin-bottom: 16px; border-left: 4px solid var(--primary); transition: 0.3s;
          position: relative;
        }
        ...
        .btn-action { background: none; border: none; padding: 4px 8px; cursor: pointer; border-radius: 4px; transition: 0.2s; }
      ...
      <div class="header">
        <div>
          <h1 style="color:var(--text-main); margin-bottom:4px; font-size:1.8rem;">Global Feed</h1>
          <p style="color:var(--text-dim); font-size:0.85rem;">실시간으로 소통하는 커뮤니티</p>
        </div>
        ${this.currentUser ? `
          <div class="user-info">
      ...
      ` : `
        <div class="login-prompt">
          메시지를 작성하려면 <span id="prompt-login-btn" class="btn-login-redirect">로그인</span>이 필요합니다.
        </div>
      `}
  ...
  setupEventListeners() {
    ...
    const handleLoginClick = () => {
      // 로그인 화면을 띄우기 위해 커스텀 이벤트를 발생시킵니다.
      ...
      submitBtn.onclick = async () => {
        ...
        try {
          await addDoc(collection(db, "comments"), {
            content: text,
            authorEmail: this.currentUser.email || this.currentUser.displayName || "익명",
            authorUid: this.currentUser.uid,
            createdAt: serverTimestamp()
          });
          input.value = '';
        } catch (e) {
          alert("전송 중 오류가 발생했습니다. 권한을 확인해 주세요.");
        }
      };
    }
  }
  ...
  loadComments() {
    ...
    onSnapshot(q, (snapshot) => {
      listEl.innerHTML = '';
      if (snapshot.empty) {
        listEl.innerHTML = '<p style="text-align:center; color:var(--text-dim)">첫 메시지를 남겨보세요!</p>';
        return;
      }
      snapshot.forEach((docSnap) => {
        ...
        item.innerHTML = `
          <div class="author">
            <span>${data.authorEmail}${isMine ? ' <span style="color:var(--accent); font-size:0.7rem;">(나)</span>' : ''}</span>
            <span class="timestamp">${date}</span>
          </div>
          <div class="content" id="content-${id}">${this.escapeHTML(data.content)}</div>
          ${isMine ? `
            <div class="actions">
              <button class="btn-action btn-edit" data-id="${id}">수정</button>
              <button class="btn-action btn-delete" data-id="${id}">삭제</button>
            </div>
          ` : ''}
        `;
        ...
        // 수정/삭제 이벤트 연결
        if (isMine) {
          item.querySelector('.btn-delete').onclick = () => this.deleteComment(id);
          item.querySelector('.btn-edit').onclick = () => this.startEdit(id, data.content);
        }
      });
    });
  }
  ...
  async startEdit(id, oldContent) {
    ...
    contentEl.innerHTML = `
      <textarea id="edit-input-${id}" style="min-height:60px; margin-top:10px;">${oldContent}</textarea>
      <div style="display:flex; gap:8px; justify-content:flex-end;">
        <button id="cancel-edit-${id}" class="btn-action" style="color:var(--text-dim)">취소</button>
        <button id="save-edit-${id}" class="btn-action btn-edit" style="font-weight:700">저장</button>
      </div>
    `;

    this.shadowRoot.getElementById(`cancel-edit-${id}`).onclick = () => contentEl.innerHTML = originalHTML;
    this.shadowRoot.getElementById(`save-edit-${id}`).onclick = async () => {
...
/* 로그인 화면 컴포넌트 (모달 형식으로 작동 가능하게 변경) */
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .login-card {
          ...
          width: 90%; max-width: 400px; box-shadow: var(--shadow-deep);
        ...
        .btn-close { position: absolute; top: 20px; right: 20px; color: #fff; cursor: pointer; font-size: 1.5rem; background: none; border: none; }
      ...
      <div class="overlay">
        <div class="login-card">
          ...
          ${this.mode === 'login' ? `
            ...
            <button id="google-btn" class="btn-google">Google로 계속하기</button>
            <div style="text-align:center; margin-top:15px;"><a id="forgot-link">비밀번호를 잊으셨나요?</a></div>
          ` : ''}

          <div class="footer">
            ${this.mode === 'login' ? '계정이 없으신가요?' : '이미 계정이 있으신가요?'}
            <a id="toggle-link">${this.mode === 'login' ? '회원가입' : '로그인으로 돌아가기'}</a>
          </div>
        </div>
      </div>
    `;
    ...
    if (form) {
      form.onsubmit = async (e) => {
        ...
        try {
          ...
          errorEl.textContent = "인증 오류가 발생했습니다.";
        }
      };
    }
  }
...
document.body.innerHTML = `
  ...
  <!-- 기존 배경 디자인 유지 -->
  <div style="position: fixed; top: -10%; left: -10%; width: 50%; height: 50%; background: var(--primary); filter: blur(150px); opacity: 0.15; border-radius: 50%; pointer-events: none; z-index: -1;"></div>
  <div style="position: fixed; bottom: -10%; right: -10%; width: 40%; height: 40%; background: var(--accent); filter: blur(150px); opacity: 0.1; border-radius: 50%; pointer-events: none; z-index: -1;"></div>
`;
