...
class ProfileSection extends HTMLElement {
  ...
  render() {
    ...
    if (!user) {
      this.shadowRoot.innerHTML = '<p style="text-align:center; padding:50px; color:var(--text-dim);">로그인이 필요합니다.</p>';
      return;
    }
    this.shadowRoot.innerHTML = `
      ...
      <div class="profile-card">
        <h2>프로필 설정</h2>
        <p style="color:var(--text-dim); font-size:0.8rem; margin-bottom:20px;">${user.providerData[0]?.providerId === 'google.com' ? '구글 계정으로 로그인됨' : '이메일 계정으로 로그인됨'}</p>
...
class CommentsSection extends HTMLElement {
  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .btn-outline { background: transparent; border: 2px solid var(--primary); color: var(--primary); padding: 8px 16px; border-radius: 8px; cursor: pointer; font-weight: 700; transition: 0.3s; }
      ...
      ` : `
        <div class="comment-input-card" style="text-align:center;"><p style="color:#ff4d4d; margin-bottom:10px;">⚠️ 이메일 인증이 필요합니다.</p><button id="resend-verify" class="btn-outline" style="font-size:0.8rem;">인증 메일 재발송</button></div>
  ...
  async postComment(inputEl, pid = null) {
    ...
    try {
      await addDoc(collection(db, "comments"), { content: text, authorEmail: this.currentUser.email, authorName: this.currentUser.displayName || "익명", authorUid: this.currentUser.uid, createdAt: serverTimestamp(), parentId: pid, likes: [] });
      inputEl.value = '';
  ...
  loadComments() {
    ...
    onSnapshot(query(collection(db, "comments"), orderBy("createdAt", "asc")), (snapshot) => {
      ...
      listEl.innerHTML = parents.length === 0 ? '<p style="text-align:center; color:var(--text-dim)">이야기를 시작해 보세요!</p>' : '';
  ...
  renderItem(container, data, isReply) {
    ...
    item.innerHTML = `
      <div class="author"><span>${data.authorName || '익명'}${isMine ? ' (나)' : ''}</span><span style="font-size:0.7rem; font-weight:400; color:var(--text-dim)">${data.createdAt ? new Date(data.createdAt.seconds * 1000).toLocaleString() : '방금 전'}</span></div>
      <div class="content" id="content-${id}">${this.escapeHTML(data.content)}</div>
      <div class="footer-actions" id="actions-${id}">
        <div class="action-link like-btn ${isLiked ? '' : 'not-liked'}" id="like-${id}">❤️ 좋아요 <span>${data.likes?.length || 0}</span></div>
    ...
    if (isMine) {
      this.shadowRoot.getElementById(`del-${id}`).onclick = async () => { if (confirm("삭제하시겠습니까?")) await deleteDoc(doc(db, "comments", id)); };
  ...
  showReplyBox(pid) {
    ...
    box.innerHTML = `<div style="margin-top:15px;"><textarea id="rin-${pid}" placeholder="답글을 남겨주세요..." style="min-height:60px;"></textarea><div style="display:flex; justify-content:flex-end; gap:10px;"><button class="btn-outline" style="font-size:0.8rem; padding:5px 12px;" id="rcan-${pid}">취소</button><button class="btn-post" style="font-size:0.8rem; padding:5px 12px;" id="rsub-${pid}">등록</button></div></div>`;
